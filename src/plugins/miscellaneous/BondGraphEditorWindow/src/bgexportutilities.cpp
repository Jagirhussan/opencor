...
void toJson(nlohmann::json &j, const QByteArray &p)
{
    j = std::string(p.constData(), p.length());
}

void fromJson(const nlohmann::json &j, QByteArray &p)
{
    ...
    for (const auto &js : jSceneItems.items()) {
        ...
        if (type == "BGDConnection") {
             ...
             */
                        ...
                        throw std::runtime_error("File is corrupt! Port " + p1.toStdString() + " does not exist!");
                    }
                }
            }
            if (elements[QString::fromStdString(definition["second"])]) {
                ...
                if (elements[QString::fromStdString(definition["second"])]->getPort(
                    ...
                    if (elements[QString::fromStdString(definition["second"])]
                        ...
                        throw std::runtime_error("File is corrupt! Port " + p2.toStdString() + " does not exist!");
                    }
                }
            }
...
loadBondGraph(nlohmann::json &res)
{
    ...
    for (const auto &itm : items.items()) {
        ...
        if (val["typeId"] == "BGElement") {
            ...
            // Find cellml
            if (clas == "passive") { // If passive set prescribed state and parameter values
            ...
            if (anot.contains("proxy")) {
                ...
                if (!link) {
                    ...
                    if (!std::get<0>(resB).is_null()) {
                        if (bondgraphImportName.find(basename) != bondgraphImportName.end()) {
                              ...
                              QDir dir, QDir ldir)
{
    ...
    for (auto &f : imports.toStdMap()) {
        try {
            std::string destString = dir.filePath(f.first).toStdString();
            std::string srcString = f.second.toStdString();
            if (srcString != destString) {
                std::ifstream src(srcString.c_str());
                std::ofstream dest(destString.c_str());
                dest << src.rdbuf();
                dest.close();
                src.close();
            }
                ...
                if (bondgraphInstances.find(basefname) != bondgraphInstances.end()) {
                    ...
                    if (proxy["link"]) {
                        ...
                        if (links.find(elementName) != links.end()) {
                            ...
                            bgraph = getBondGraphFor(j["sceneItems"], import_file);
