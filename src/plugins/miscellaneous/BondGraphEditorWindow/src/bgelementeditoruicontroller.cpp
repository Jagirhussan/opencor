...
BGElementEditorUIController::BGElementEditorUIController(
    BondGraphUIWidget *parent, bool p_doBackup) :
    QObject(nullptr),
    m_parent(parent)
{
    // create document
...
void BGElementEditorUIController::setAnnotationEditor(
    BGAnnotationViewMetadataEditDetailsUI *p_editor)
{
    m_annotationEditor = p_editor;
...
void BGElementEditorUIController::onSceneDoubleClicked(
    QGraphicsSceneMouseEvent * /*mouseEvent*/, QGraphicsItem *p_clickedItem)
{
    BGPort *port = dynamic_cast<BGPort *>(p_clickedItem);
    if (port)
        editNodePort(*port);
}
...
void BGElementEditorUIController::onDocumentLoaded(const QString &p_fileName)
{
    ...
    QString filename = QFileInfo(p_fileName).fileName();
...
void BGElementEditorUIController::updateSceneOptions()
{
    if (m_optionsData.backupPeriod > 0) {
        m_backupTimer.setInterval(m_optionsData.backupPeriod * 6000); // 1 minute - m_optionsData.backupPeriod =
...
void BGElementEditorUIController::onZoomChanged(double p_currentZoom)
{
    Q_UNUSED(p_currentZoom);
    fitZoomBackAction->setEnabled(m_editorView->getZoomBeforeFit() > 0);
}

void BGElementEditorUIController::zoom()
{
    m_editorView->zoomBy(1.3);
}

void BGElementEditorUIController::unzoom()
{
    m_editorView->zoomBy(1.0 / 1.3);
}

void BGElementEditorUIController::resetZoom()
{
...
void BGElementEditorUIController::showElementIds(bool p_on)
{
    m_editorScene->setClassAttributeVisible(class_element, attr_id, p_on);

    m_editorScene->addUndoState();
}

void BGElementEditorUIController::showConnectionIds(bool p_on)
{
    m_editorScene->setClassAttributeVisible(class_connection, attr_id, p_on);
...
void BGElementEditorUIController::copy()
{
    m_editorScene->copy();
}
void BGElementEditorUIController::cut()
{
    m_editorScene->cut();
}
void BGElementEditorUIController::paste()
{
    m_editorScene->paste();
}
void BGElementEditorUIController::del()
{
    m_editorScene->del();
}
void BGElementEditorUIController::selectAll()
{
    m_editorScene->selectAll();
}
